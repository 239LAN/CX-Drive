...
- **Background remote transfer**: reads and writes on remote storage points (FTP / SFTP / S3) run as background tasks — uploads return immediately, while downloads / previews / unzip jump to a live progress page, and the file list marks files as "fetching / uploading". A storage point that fails a read or write is taken out of rotation and shown in the admin console with the reason
