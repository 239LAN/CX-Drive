# -*- coding: utf-8 -*-
"""整体回归冒烟测试：在隔离的临时 DB/存储上验证用户端、分享与管理端全链路渲染及关键安全约束"""
import os
import sys
import io
import shutil
...
def reg(c, u):
    return c.post("/register", data={"username": u, "email": f"{u}@t.cn", "agree": "on",
                                     "password": "pass1234", "confirm": "pass1234"},
                  follow_redirects=False)


def login(c, u):
    return c.post("/login", data={"username": u, "password": "pass1234", "agree": "on"},
                  follow_redirects=False)
